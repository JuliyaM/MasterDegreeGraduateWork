...
class ProjectWithEndFingWeightsAlgorithmFragment(
    ...
    override fun getFragment(): FlowContent.() -> Unit = {
        with(withEndProcessWeightSolveResult) {
            ...
            p {
                +"""Данное решение не учитывает начальное состояние системы. Чтобы это исправить и задать i-e начальное состояние, необходимо 
                совершить шаг системы (возвести матрицу в квадрат) и заменить i-ю строку на i-ю строку из начальной матрицы.
